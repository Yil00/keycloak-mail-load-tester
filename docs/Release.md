...
- **Scripts** : déplacés dans `src/` :
  ...
  - `src/keycloak_admin_utils.py` : créer un superadmin, lister le nombre d'utilisateurs par realm, supprimer les utilisateurs de test uniquement (loadtest_* et testuser_*), **créer les comptes loadtest_user_1..N pour Locust** (sans toucher à admin ni aux autres).
- **Locust** : tests de charge avec un compte distinct par utilisateur virtuel (password grant, logout en fin de test). Service `locust` dans docker-compose (port 8089), scénario dans `locust/locustfile.py`. Commandes : `make create-locust-users`, `make locust-headless`, `make locust-trigger`. Doc : [docs/locust.md](locust.md), `locust/README.md`. Variables : `env.dist` (section Locust), `LOCUST_PORT`, `KEYCLOAK_CLIENT_ID`, `KEYCLOAK_LOAD_*`, `KEYCLOAK_USER_COUNT`.
...
- **Documentation** : `docs/admin-keycloak.md`, `docs/admin-utils.md` (superadmin, list-users, delete-test-users), `docs/load-test-tokens.md`, `docs/session-exporter.md`, `docs/locust.md` (Locust, comptes distincts), `docs/grafana-metrics.md`, `docs/grafana-dashboards.md`, `docs/prometheus.md`. `grafana/README.md`, `prometheus/README.md`, `locust/README.md`. `env.dist` documente `KEYCLOAK_PORT`, `EXPORTER_PORT`, section Locust (`LOCUST_PORT`, `KEYCLOAK_CLIENT_ID`, `KEYCLOAK_LOAD_*`, `KEYCLOAK_USER_COUNT`) et variables admin (SUPERADMIN_*, REALM).
